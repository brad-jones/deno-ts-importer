/**
 * Performance Benchmark Summary for TsImporter
 *
 * This benchmark compares TsImporter performance against native dynamic import.
 */
...
console.log("🚀 TsImporter Performance Benchmarks\n");

// 1. First Import Performance (Cold Cache)
Deno.bench({
  name: "First Import: TsImporter (transforms & caches)",
  group: "first-import",
  async fn() {
    await cleanup();
...
Deno.bench({
  name: "Complex Graph: TsImporter (nested deps)",
...
Deno.bench({
  name: "No Imports: TsImporter overhead",
  group: "no-imports",
  async fn() {
    await cleanup();
    const importer = new TsImporter({
